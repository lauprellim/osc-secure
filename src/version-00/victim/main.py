...
- OSC: /note (midi_note, velocity) over UDP to receiving end

Wiring:
- Button between GPIO15 and GND (using the internal pull-up resistor)
- LED anode -> GPIO16 through resistor, cath -> GND
...
def main():
    ...
    seq = build_major_arpeggio(48)  # C3
    idx = 0
    ...
    while True:
        ...
        if sending:
            ...
            if time.ticks_diff(now, next_send_ms) >= 0:
                note = seq[idx]
                pkt = osc_pack('/note', ',ii', (note, VELOCITY))
